      ...
      schemas: {
        ...
        RescueRequest: {
          ...
          properties: {
            ...
            tracking_status: {
              type: "string",
              enum: ["submitted", "received", "assigned", "team_ready", "en_route", "completed", "incident_reported"],
              description: "Trạng thái hiển thị cho citizen (tách biệt với status). Flow: submitted→received→assigned→team_ready→en_route→completed hoặc incident_reported",
            },
            incident_report_note: { type: "string", nullable: true, description: "Ghi chú sự cố khi tracking_status = incident_reported" },
            incident_team_info: { type: "string", nullable: true, description: "Thông tin đội khi có sự cố (JSON string)" },
          },
        },
        CreateRescueRequest: {
          type: "object",
    ...
    paths: {
      ...
      "/requests/map": {
        get: {
          ...
          description: "Không cần xác thực. Trả về tọa độ + tracking_status để màu marker và badge nhất quán với trang theo dõi.",
          parameters: [
            { name: "province_id", in: "query", schema: { type: "integer" } },
          ...
          responses: {
            200: {
              description: "Danh sách yêu cầu kèm tọa độ cho bản đồ",
              content: {
                "application/json": {
                  schema: {
                    type: "array",
                    items: {
                      ...
                      properties: {
                        ...
                        latitude: { type: "number" },
                        longitude: { type: "number" },
                        status: { type: "string", description: "Trạng thái nội bộ" },
                        tracking_status: {
                          type: "string",
                          enum: ["submitted", "received", "assigned", "team_ready", "en_route", "completed", "incident_reported"],
                          description: "Trạng thái citizen-facing — dùng để màu marker bản đồ và badge danh sách",
                        },
                        victim_count: { type: "integer" },
                        incident_type: { type: "string" },
                        urgency_level: { type: "string" },
                        urgency_color: { type: "string" },
                        province_name: { type: "string" },
      ...
      "/requests/{id}/close": {
        put: {
          tags: ["Requests"],
          summary: "Coordinator đóng đơn cứu hộ sau khi hoàn thành",
          description: "Yêu cầu rescue_team_confirmed = true. Cập nhật status → 'completed', ghi response_time_minutes, giảm workload coordinator, gửi notification cho citizen.",
          security: [{ bearerAuth: [] }],
          parameters: [
            { name: "id", in: "path", required: true, schema: { type: "integer" }, description: "rescue_request ID" },
          ],
          responses: {
            200: { description: "Đóng đơn thành công" },
            400: { description: "Đội cứu hộ chưa xác nhận hoàn thành hoặc đơn đã đóng" },
      ...
      "/tasks": {
        ...
        post: {
          tags: ["Tasks"],
          summary: "Tạo task mới (Coordinator)",
          security: [{ bearerAuth: [] }],
          requestBody: {
            required: true,
            content: {
              "application/json": {
                schema: {
                  type: "object",
                  required: ["name", "team_id", "request_ids"],
                  properties: {
                    name: { type: "string", example: "Task cứu hộ Bình Thạnh" },
                    team_id: { type: "integer" },
          ...
          responses: { 201: { description: "Tạo task thành công" } },
        },
      },
      "/tasks/{id}": {
        get: {
          tags: ["Tasks"],
          summary: "Chi tiết task (gồm missions, incident_reports, resources)",
          security: [{ bearerAuth: [] }],
                ...
                "application/json": {
                  schema: {
                    ...
                    properties: {
                      id: { type: "integer" },
                      name: { type: "string" },
                      status: { type: "string", enum: ["in_progress", "completed", "partial", "cancelled"] },
                      ...
                      incident_reports: {
                        ...
                        items: {
                          ...
                          properties: {
                            id: { type: "integer" },
                            request_id: { type: "integer", description: "ID của rescue_request liên quan — dùng để gọi PUT /requests/{id}/close" },
                            tracking_code: { type: "string" },
                            report_type: { type: "string", enum: ["stalled", "unrescuable", "need_support"] },
                            urgency: { type: "string" },
      ...
      "/tasks/{id}/reports": {
        post: {
          tags: ["Tasks"],
          summary: "Team gửi báo cáo sự cố (Rescue Team)",
          description: "Khi report_type = 'unrescuable', hệ thống tự động cập nhật tracking_status của rescue_request sang 'incident_reported'",
          security: [{ bearerAuth: [] }],
          parameters: [{ name: "id", in: "path", required: true, schema: { type: "integer" }, description: "task_group_id" }],
          requestBody: {
            required: true,
            content: {
              "application/json": {
                schema: {
                  type: "object",
                  required: ["mission_id", "report_type", "urgency", "description"],
                  properties: {
                    mission_id: { type: "integer" },
                    report_type: { type: "string", enum: ["stalled", "unrescuable", "need_support"] },
          ...
          responses: {
            201: { description: "Báo cáo đã được ghi nhận, socket event 'task_incident_report' được emit" },
            404: { description: "Task không tồn tại" },
          },
        },
      },
      "/tasks/{id}/reports/{reportId}/resolve": {
        put: {
          tags: ["Tasks"],
          summary: "Coordinator xử lý báo cáo sự cố",
          security: [{ bearerAuth: [] }],
          parameters: [
      ...
      // ==================== CONFIG ====================
      // ==================== CONFIG — DANH MỤC (Admin) ====================
      "/config/incident-types": {
        get: {
          tags: ["Config"],
          summary: "Danh sách loại sự cố",
          security: [{ bearerAuth: [] }],
          responses: { 200: { description: "Danh sách incident_types" } },
        },
        post: {
          tags: ["Config"],
          summary: "Tạo loại sự cố mới (Admin)",
          ...
          requestBody: {
            ...
            content: {
              "application/json": {
                schema: {
                  type: "object",
                  required: ["name", "rescue_category"],
                  properties: {
                    name: { type: "string", example: "Ngập lụt" },
                    code: { type: "string", example: "flood" },
                    icon: { type: "string", example: "water" },
                    color: { type: "string", example: "#2196F3" },
                    description: { type: "string" },
                    rescue_category: { type: "string", enum: ["cuu_nan", "cuu_tro", "cuu_ho"] },
                  },
                },
              },
            },
      ...
      "/config/incident-types/{id}": {
        ...
        delete: {
          tags: ["Config"],
          summary: "Xóa loại sự cố (Admin)",
          security: [{ bearerAuth: [] }],
          parameters: [{ name: "id", in: "path", required: true, schema: { type: "integer" } }],
          responses: { 200: { description: "Xóa thành công" } },
        },
      },
      "/config/urgency-levels": {
        get: {
          tags: ["Config"],
          summary: "Danh sách mức độ khẩn cấp",
          security: [{ bearerAuth: [] }],
          responses: { 200: { description: "Danh sách urgency_levels" } },
        },
        post: {
          tags: ["Config"],
          summary: "Tạo mức độ khẩn cấp mới (Admin)",
          security: [{ bearerAuth: [] }],
          requestBody: {
            required: true,
            content: {
              "application/json": {
                schema: {
                  type: "object",
                  required: ["name", "priority_score"],
                  properties: {
                    name: { type: "string", example: "Khẩn cấp" },
                    code: { type: "string", example: "critical" },
                    priority_score: { type: "integer", example: 100 },
                    color: { type: "string", example: "#F44336" },
                    max_response_minutes: { type: "integer", example: 30 },
                    description: { type: "string" },
                  },
                },
              },
      ...
      "/config/urgency-levels/{id}": {
        put: {
          tags: ["Config"],
          summary: "Cập nhật mức độ khẩn cấp (Admin)",
          security: [{ bearerAuth: [] }],
          parameters: [{ name: "id", in: "path", required: true, schema: { type: "integer" } }],
          requestBody: {
            required: true,
        ...
        delete: {
          tags: ["Config"],
          summary: "Xóa mức độ khẩn cấp (Admin)",
          security: [{ bearerAuth: [] }],
          parameters: [{ name: "id", in: "path", required: true, schema: { type: "integer" } }],
          responses: { 200: { description: "Xóa thành công" } },
        },
      ...
      "/config/relief-items": {
        get: {
          tags: ["Config"],
          summary: "Danh sách vật tư cứu trợ",
          security: [{ bearerAuth: [] }],
          responses: { 200: { description: "Danh sách relief_items" } },
        },
        post: {
          tags: ["Config"],
          summary: "Tạo vật tư cứu trợ mới (Admin)",
          security: [{ bearerAuth: [] }],
          requestBody: {
            required: true,
            content: {
              "application/json": {
                schema: {
                  ...
                  properties: {
                    name: { type: "string", example: "Gạo" },
                    category: { type: "string", enum: ["food", "water", "medical", "equipment", "shelter", "fuel"] },
                    unit: { type: "string", example: "kg" },
                    description: { type: "string" },
                    rescue_category: { type: "string", enum: ["cuu_nan", "cuu_tro", "cuu_ho", "all"] },
                  },
                },
              },
            },
      ...
      "/config/relief-items/{id}": {
        put: {
          tags: ["Config"],
          summary: "Cập nhật vật tư cứu trợ (Admin)",
          security: [{ bearerAuth: [] }],
          parameters: [{ name: "id", in: "path", required: true, schema: { type: "integer" } }],
          requestBody: {
            required: true,
        ...
        delete: {
          tags: ["Config"],
          summary: "Xóa vật tư cứu trợ (Admin)",
          security: [{ bearerAuth: [] }],
          parameters: [{ name: "id", in: "path", required: true, schema: { type: "integer" } }],
          responses: { 200: { description: "Xóa thành công" } },
        },
